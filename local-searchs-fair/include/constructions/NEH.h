...
class NEH {
  public:
    ...
    // Same as taillard_best_insertion but splits the position loop around
    // `original_position`, skipping that slot (used by TPA's Simulated Annealing
    // to generate a move that is guaranteed to differ from the current sequence).
    // Ported from TPA/src/constructions/NEH.cpp.
    std::pair<size_t, size_t> mtaillard_best_insertion(const std::vector<size_t> &s, size_t k,
