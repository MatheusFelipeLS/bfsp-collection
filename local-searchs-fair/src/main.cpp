...
// this is the "equal time budget" variant, see README.md "Comparação justa"
// for how it differs from local-searchs/.
//
//   ./local-searchs-fair <instance>                      # benchmark every local search
//   ./local-searchs-fair <instance> -p rls-grabowski      # just one
//   ./local-searchs-fair <instance> -n 20 -s 1            # 20 reps, RNG seed 1
//   ./local-searchs-fair <instance> --time-limit-ms 5000  # bigger shared budget
//
...
// given seed) AND runs under the SAME `--time-limit-ms` wall-clock budget
// (restarting with a freshly reshuffled move order until a pass stops
// improving or the budget runs out - see Harness.cpp's run_ref_to_budget/
// run_call_to_budget), so improvement/time are directly comparable across
// entries. See README.md for the full catalog and deduplication rules.
