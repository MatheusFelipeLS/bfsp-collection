...
Solution GRASP_NEH::solve() {
    ...
    const std::vector<size_t> phi = {pi_h.sequence.begin() + (long)m_x, pi_h.sequence.end()};

    pi_h.sequence.erase(pi_h.sequence.begin() + (long)m_x, pi_h.sequence.end());
...
Solution GRASP_NEH::GRASP() {
    ...
    for (size_t i = 1; i < m_instance.num_jobs(); i++) {
        ...
        for (size_t j = 0; j < m_instance.num_jobs(); j++) {
            ...
            std::vector<size_t> dp = core::calculate_new_departure_time(m_instance, s.departure_times, jobs[j]);
            c[j] = core::calculate_alpha(m_instance, s.departure_times, dp, jobs[j], i);
        ...
        for (size_t j = 0; j < m_instance.num_jobs(); j++) {
            ...
            if ((double)c[j] <= accept_criterion) {
                rcl.push_back(j);
            }
        }
